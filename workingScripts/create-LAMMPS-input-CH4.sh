...
pressureList="10000 25000 50000 100000 150000 200000 250000 300000 350000 400000 450000 500000 1000000 1500000 2000000 2500000 3000000 3500000 4000000 4500000 5000000 10000000"
...
do
  ...
  do
    ...
    uptake=($(awk '$1 == "'"$pressure"'"' isotherms/${metal}_${guest}_${tempInt}_absolute.txt))
    ...
    if [ $guest = "CH4" ]; then
      :
        ...
        echo " Bonds" >> ${metal}MOF74-Pressure${pressure}.data
        echo " " >> ${metal}MOF74-Pressure${pressure}.data
        count=1
        for bondNumber in $(seq 1 $replicasGuest)
        do
          if [ $bondNumber -eq 1 ]; then
            echo "$count 1 1 2" >> ${metal}MOF74-Pressure${pressure}.data
            count=$(echo "$count + 1" | bc -l)
            echo "$count 1 2 3" >> ${metal}MOF74-Pressure${pressure}.data
            count=$(echo "$count + 1" | bc -l)
          elif [ $bondNumber -gt 1 ]; then
            echo "$count 1 $(echo "1 + 3 * ($bondNumber - 1)" | bc -l) $(echo "2 + 3 * ($bondNumber - 1)" | bc -l)" >> ${metal}MOF74-Pressure${pressure}.data
            count=$(echo "$count + 1" | bc -l)
            echo "$count 1 $(echo "2 + 3 * ($bondNumber - 1)" | bc -l) $(echo "3 + 3 * ($bondNumber - 1)" | bc -l)" >> ${metal}MOF74-Pressure${pressure}.data
            count=$(echo "$count + 1" | bc -l)
          fi
        done
        twiceBondNumber=$(echo "$bondNumber * 2" | bc -l)
        sed -i "s/0 bonds/$twiceBondNumber bonds/" ${metal}MOF74-Pressure${pressure}.data
        sed -i "s/0 bond types/1 bond types/" ${metal}MOF74-Pressure${pressure}.data
        echo " " >> ${metal}MOF74-Pressure${pressure}.data
        echo " Bond Coeffs" >> ${metal}MOF74-Pressure${pressure}.data
        echo " " >> ${metal}MOF74-Pressure${pressure}.data
        echo "1 351.487 1.44" >> ${metal}MOF74-Pressure${pressure}.data
    elif [ $guest = "H2O" ]; then
      :
    fi
    # prepare in file
    ...
    if [ $guest = "CH4" ]; then
      ...
      sed -i "s/kspace_style       ewald 1.0e-6/#kspace_style       ewald 1.0e-6/g" ${metal}MOF74-Pressure${pressure}.in
      sed -i "s/fix                5/#fix                5/g" ${metal}MOF74-Pressure${pressure}.in
      sed -i "s/bond_style/#bond_style/g" ${metal}MOF74-Pressure${pressure}.in
      sed -i "s/bond_coeff/#bond_coeff/g" ${metal}MOF74-Pressure${pressure}.in
      sed -i "s/comm_modify/#comm_modify/g" ${metal}MOF74-Pressure${pressure}.in
    elif [ $guest = "CO2" ]; then
      ...
      sed -i "s/dump_modify        3/#dump_modify        3/g" ${metal}MOF74-Pressure${pressure}.in
    elif [ $guest = "H2O" ]; then
      cp forceFieldParamsTemplates/forceFieldParams-${metal}MOF74-charged-template ${metal}MOF74-Pressure${pressure}.in.settings
      #sed -i "s/compute            ChunkGuest/#compute            ChunkGuest/g" ${metal}MOF74-Pressure${pressure}.in
      #sed -i "s/compute            comChunkGuest/#compute            comChunkGuest/g" ${metal}MOF74-Pressure${pressure}.in
    ...
    sed -i "s/guestAndMofAtoms/${guestAndMofAtoms}/g" ${metal}MOF74-Pressure${pressure}.in
