...
if $fromXxToMof_Xx ; then
  if [ ${fileExt} == "pdb" ]; then
    ...
    for i in *.${fileExt}
    do 
      sed -i 's/O\s/Mof_Oa /g' $i
      sed -i 's/Br\s/Mof_Ob /g' $i
      sed -i 's/Cl\s/Mof_Oc /g' $i
      sed -i 's/C\s/Mof_Ca /g' $i
      sed -i 's/F\s/Mof_Cb /g' $i
      sed -i 's/N\s/Mof_Cc /g' $i
      sed -i 's/B\s/Mof_Cd /g' $i
      sed -i 's/H\s/Mof_H /g' $i
      sed -i 's/Co\s/Mof_Co /g' $i
      sed -i 's/Mg\s/Mof_Mg /g' $i
      sed -i 's/Mn\s/Mof_Mn /g' $i
      sed -i 's/Fe\s/Mof_Fe /g' $i
      sed -i 's/Zn\s/Mof_Zn /g' $i
      sed -i 's/Ni\s/Mof_Ni /g' $i
      sed -i 's/Kr\s/CH4_sp3 /g' $i
    done
  fi
fi
