...
isothermData-74/${metal}_${guest}_${tempInt}_absolute.txt))
      ...
      if [ $metal = "Mg" ]; then
        replicasGuest=$(echo "($frameworkReplicas)/4 * 2" | bc )
      elif [ $metal = "Ni" ]; then
        replicasGuest=$(echo "($frameworkReplicas)/4 * 2" | bc )
      elif [ $metal = "Zn" ]; then
        replicasGuest=$(echo "($frameworkReplicas)/2 * 2" | bc )
      fi
...
>> ${dataFile}
      ...
      if [ $metal = "Mg" ]; then
        cycles=$(echo "($frameworkReplicas)/ (4 * 6) * 2" | bc )
        ...
        cycles=$(echo "($frameworkReplicas)/ (2 * 12) * 2" | bc )
        for number in $(seq 1 $cycles); do 
