...
do
  ...
  do
    ...
    toposcript="toposcript-${metal}MOF274-Pressure${pressure}.tcl"

    # create .tcl topos script
    if [ $guest = "CH4" ]; then
      cp topoScripts/toposcript-${metal}MOF274-uncharged.tcl ${toposcript}
      cp topoScripts/toposcript-assign-molecules.py .
    elif [ $guest = "CO2" ]; then
      cp topoScripts/toposcript-${metal}MOF274-charged.tcl ${toposcript}
      cp topoScripts/toposcript-assign-molecules.py .
    elif [ $guest = "H2O" ]; then
      cp topoScripts/toposcript-${metal}MOF274-charged.tcl ${toposcript}
      cp topoScripts/toposcript-assign-molecules.py .
...
isothermData-274/${metal}_${guest}_${tempInt}_absolute.txt))
    ...
    # define filenames
    dataFile="${metal}MOF274-Pressure${pressure}.data"
    inFile="${metal}MOF274-Pressure${pressure}.in"
    settingsFile="${metal}MOF274-Pressure${pressure}.in.settings"
...
>> ${dataFile}
    ...
    if [ $guest = "CH4" ]; then
      cp \
forceFieldParamsTemplates/forceFieldParams-${metal}MOF274-uncharged-template \
${settingsFile}
      ...
      cp \
forceFieldParamsTemplates/forceFieldParams-${metal}MOF274-charged-template \
${settingsFile}
      ...
      sed -i "s/#fix                3/fix                3/g" ${inFile}
      sed -i "s/#fix_modify         3/fix_modify         3/g" ${inFile}
      sed -i "s/dump               3/#dump               3/g" ${inFile}
      sed -i "s/dump_modify        3/#dump_modify        3/g" ${inFile}
    elif [ $guest = "H2O" ]; then
      cp forceFieldParamsTemplates/forceFieldParams-${metal}MOF274-charged-template ${settingsFile}
      #sed -i "s/compute            ChunkGuest/#compute            ChunkGuest/g" ${inFile}
      #sed -i "s/compute            comChunkGuest/#compute            comChunkGuest/g" ${inFile}
      #sed -i "s/fix                3/#fix                3/g" ${inFile}
      #sed -i "s/fix                4/#fix                4/g" ${inFile}
...
mkdir MOF-274-CO2-input-files/                                                   
mv *Pressure* MOF-274-CO2-input-files/                                           
echo "Done! Input files can be found in MOF-274-CO2-input-files/"
